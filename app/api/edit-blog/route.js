import { NextResponse } from 'next/server'
import connectDB from '@/app/lib/db'
import BlogPost from '@/app/model/BlogPost'
import cloudinary from '@/app/lib/cloudinary'
import slugify from 'slugify'

export async function PUT(request) {
    try {
        await connectDB()
        const body = await request.json()
        const { originalBlogId, title, category, author, sections } = body

        if (!originalBlogId || !title || !category || !author || !sections?.length) {
            return NextResponse.json({ message: 'Invalid blog data.' }, { status: 400 })
        }

        const blog = await BlogPost.findOne({ blogId: originalBlogId })
        if (!blog) {
            return NextResponse.json({ message: 'Blog not found.' }, { status: 404 })
        }

        const newBlogId = slugify(title, {
            lower: true,
            strict: true,
            locale: 'en'
        })

        // Helper to upload a single base64 image to Cloudinary
        const uploadBase64 = async (base64Str) => {
            try {
                const res = await cloudinary.v2.uploader.upload(base64Str, {
                    folder: `blogs/${newBlogId}`,
                })
                return res.secure_url
            } catch (err) {
                console.error('Cloudinary upload error:', err)
                return null
            }
        }

        const updatedSections = await Promise.all(
            sections.map(async (sec) => {
                if (sec.image && sec.image.startsWith('data:')) {
                    // New base64 image uploaded
                    const url = await uploadBase64(sec.image)
                    return {
                        heading: sec.heading,
                        content: sec.content,
                        image: url,
                    }
                } else {
                    // Existing image URL, no need to upload
                    return {
                        heading: sec.heading,
                        content: sec.content,
                        image: sec.image,
                    }
                }
            })
        )

        // Update fields
        blog.blogId = newBlogId
        blog.title = title
        blog.category = category
        blog.author = author
        blog.sections = updatedSections

        await blog.save()

        return NextResponse.json({ message: 'Blog updated successfully', blog }, { status: 200 })

    } catch (error) {
        console.error('Error updating blog:', error)
        return NextResponse.json({ message: 'Internal Server Error' }, { status: 500 })
    }
}
